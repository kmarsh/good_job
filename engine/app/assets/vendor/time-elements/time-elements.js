(() => {
  ...
  var yearFormatter = makeFormatter({day: "numeric", month: "short", year: "numeric"});
  ...
  // src/extended-time-element.ts
  var datetimes = new WeakMap();
  var ExtendedTimeElement = class extends HTMLElement {
  ...
  var extended_time_element_default = ExtendedTimeElement;
  ...
  var LocalTimeElement = class extends extended_time_element_default {
    attributeChangedCallback(attrName, oldValue, newValue) {
  ...
  var local_time_element_default = LocalTimeElement;
  ...
  var relative_time_default = RelativeTime;
  ...
  var timeFormatter = makeFormatter({hour: "numeric", minute: "2-digit"});

  // src/relative-time-element.ts
  var RelativeTimeElement = class extends extended_time_element_default {
    getFormattedDate() {
      const date = this.date;
      if (!date)
        return;
      return new relative_time_default(date, localeFromElement(this)).toString();
    }
    connectedCallback() {
      nowElements.push(this);
      if (!updateNowElementsId) {
        updateNowElements();
        updateNowElementsId = window.setInterval(updateNowElements, 60 * 1e3);
      }
  ...
  var relative_time_element_default = RelativeTimeElement;
  var nowElements = [];
  ...
  var TimeAgoElement = class extends relative_time_element_default {
    getFormattedDate() {
      ...
      if (!date)
        return;
      if (format === "micro") {
        return new relative_time_default(date, localeFromElement(this)).microTimeAgo();
      } else {
        return new relative_time_default(date, localeFromElement(this)).timeAgo();
      }
    }
  };
  var time_ago_element_default = TimeAgoElement;
  ...
  var TimeUntilElement = class extends relative_time_element_default {
    getFormattedDate() {
      const format = this.getAttribute("format");
      const date = this.date;
      if (!date)
        return;
      if (format === "micro") {
        return new relative_time_default(date, localeFromElement(this)).microTimeUntil();
      } else {
        return new relative_time_default(date, localeFromElement(this)).timeUntil();
      }
    }
  };
  var time_until_element_default = TimeUntilElement;
